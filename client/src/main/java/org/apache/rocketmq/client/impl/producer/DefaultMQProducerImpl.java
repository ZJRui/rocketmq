...
public class DefaultMQProducerImpl implements MQProducerInner {
    ...
    //DefaultMQProducerImpl是MQProducerInner接口的子类， 但是这里内部存在一个DefaultMQProducer
    //这个属性是MQProducer接口的实现类。 DefaultMQProducerImpl和DefaultMQProducer在集成层次上没有任何关系，但是相互嵌套包含
    ...
    public void start(final boolean startFactory) throws MQClientException {
        switch (this.serviceState) {
            ...
            case CREATE_JUST:
                ...
                //创建MQClientManager，创建对象的时候会初始化netty相关配置，MQClientInstance
                ...
                //作为生产者和消费者的客户端启动  new DefaultMQProducer之后调用其start ->
                // MQClientInstance创建并start，此时会执行MQClientAPIImpl start，然后就会执行NettyRemotingClient。start
                //这里会启动netty
                if (startFactory) {
    ...
    private void checkConfig() throws MQClientException {
        //从这里我们可以看到 虽然我们执行了DefaultMQProducerImpl的start方法，但是关于producer的主要信息和方法还是在  DefaultMQProducer对象中的
        //可以说DefaultMQProducerImpl是封装了 producer的外部实现
    ...
    public TransactionSendResult sendMessageInTransaction(final Message msg,
        ...
        SendResult sendResult = null;
        MessageAccessor.putProperty(msg, MessageConst.PROPERTY_TRANSACTION_PREPARED, "true");//TRAN_MSG,表示消息未prepare消息
